...
import VideoModal from './components/VideoModal'; // Ajustez si vous utilisez l'ancien nom de fichier
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] text-slate-200 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      {/* SIDEBAR avec le bon Titre et les catégories */}
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto h-screen">
        <header className="flex justify-between items-center mb-8">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
             {activeTab === 'accueil' ? 'À la Une' : CATEGORIES.find(c => c.id === activeTab)?.label}
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {programs.filter(p => p.categoryId === activeTab).map(prog => (
               <div key={prog.id} onClick={() => setSelectedProg(prog)} className="group cursor-pointer">
                 {/* Si vous avez besoin de recréer les cartes ici, sinon elles s'affichent via ProgramRow sur l'accueil */}
