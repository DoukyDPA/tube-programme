...
export default function App() {
  ...
  // On récupère tous les programmes, mais on ne filtrera que ceux de l'utilisateur pour l'affichage
  useEffect(() => {
    if (!user) return;
  ...
  const syncWhatsNew = async () => {
    ...
    try {
      ...
      // On ne synchronise que les chaînes que CET utilisateur a ajoutées
      for (const p of programs.filter(p => p.addedBy === user.uid)) {
      ...
      if (channels.length === 0) return setIsSyncing(false);
      ...
      for (const channel of channels) {
        ...
        for (const v of pData.items) {
          ...
          promises.push(setDoc(newDocRef, {
            ...
            addedBy: user.uid, pitch: "", createdAt: Date.now(), avgScore: 0
          }));
          addedCount++;
          channelAddedVideos++; 
      ...
      alert(addedCount > 0 ? `✅ ${addedCount} nouvelles vidéos !` : `ℹ️ À jour.`);
  ...
  // Filtrage strict : l'utilisateur ne voit QUE ce qu'il a ajouté
